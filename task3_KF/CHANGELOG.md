...
## 2026-09-17 主装甲板选择与遮挡续跟

- 主板选择采用 sp_vision 的正面角度窗口、双板锁定和旋转进入/离开窗口；按本项目相机 x-z 平面及板法线方向计算角度。
...
- TEMP_LOST 时间上限从 1.5 秒延长到 3 秒，连续漏检上限从 15 帧提高到 120 帧；任一上限到达后失锁，原有发散保护保留。
- 未调整检测器、PnP、EKF 和数据关联算法。tracker 构建及 stability_test 通过，覆盖双板锁定、双向旋转换板、两秒遮挡恢复及超时失锁。
