...
### 新增
- 主装甲板选择与遮挡续跟
- 主板选择采用 sp_vision 的正面角度窗口、双板锁定和旋转进入/离开窗口
- 普通运动使用 ±60° 窗口；高速旋转使用 60°/20°，前哨使用 70°/30°。转速读取 x[7]，避免参考实现中以半径 x[8] 判断转速的问题。
- 主板选择与整车观测更新独立，当前主板仍可检出时也能按窗口换板，TEMP_LOST 期间继续根据预测模型选板。
- TEMP_LOST 连续漏检上限从 15 帧提高到 120 帧；和时间任一上限到达后失锁，原有发散保护保留。


## 2026-09-17 
### 新增
- 整车 EKF 动态 Q
- 在既有 CV 平移与匀角速度预测中加入可关闭的创新 NIS 倍率，可靠更新触发，下一次预测生效，并按实际时间指数回落。
- 仅调整运动噪声，保留几何参数过程噪声、关联门控、NIS 门控和物理发散保护；不新增算法模块。
- tracker.yaml 提供开关、NIS 基准、倍率上限、回落时间常数；HUD 显示模式、倍率和 NIS，支持固定/动态 Q 视频对比。
