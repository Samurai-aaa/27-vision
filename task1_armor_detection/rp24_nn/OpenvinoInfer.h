...
public:
    ...
    OpenvinoInfer(string model_path, string device){
        // 模型实际输入为 NCHW [1,3,640,640]（0526.onnx: fp16 / 0708.onnx: fp32）。
        // 此处复用下方 xml+bin 多文件构造的预处理方案：
        // user tensor 按 u8 NHWC {1,640,640,3} BGR（与主程序传入的 640x640x3 BGR Mat 内存布局一致），
        // 经 convert_element_type -> convert_color(RGB) -> scale(1/255) 对齐模型 NCHW 输入。
